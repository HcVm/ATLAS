        ...
        <div class="content">
          ...
          <div class="section">
            ...
            <div class="section-content">
              ...
              <div class="value">${data.contactPerson || "N/A"}</div>
            </div>
          </div>
        </div>
        ...
        <div class="footer">
          ...
          <div class="bank-section">
            ...
            <div class="bank-info">
              ...
              Cotización válida según términos y condiciones de GALUR. Confirmar disponibilidad de stock.
            </div>
          </div>
        </div>
      </div>
...
export const generateGALURPrivateQuotationPDF = async (data: GALURPrivateQuotationPDFData): Promise<void> => {
  try {
    console.log("🚀 Iniciando generación de PDF GALUR Privado...")

    const qrCodeBase64 = await generateQRForGALURQuotation(data.quotationNumber, data)
    ...
    const response = await fetch("/api/generate-quotation-pdf", {
      ...
      body: JSON.stringify({ pdfData: data, type: "galur-private" }),
    })
    ...
    a.href = url
    a.download = `Cotizacion_GALUR_Privada_${data.quotationNumber.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`
    document.body.appendChild(a)
    a.click()
    a.remove()
    window.URL.revokeObjectURL(url)

    console.log("✅ PDF GALUR Privado generado exitosamente")
  } catch (error) {
