...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    try {
      ...
      // Fetch movements con la misma lógica corregida que en movements/page.tsx
      let allMovements: any[] = []
      const isAdmin = user?.role === "admin" || user?.role === "supervisor"

      if (user && !isAdmin && user.department_id) {
        // Múltiples consultas para usuarios no admin
        const baseSelect = `
          *,
        ...
        const [userCreatedRes, fromDeptRes, toDeptRes] = await Promise.all([
          // Movimientos de documentos creados por el usuario
          supabase
          ...
          // Movimientos desde el departamento del usuario
          supabase
            .from("document_movements")
            .select(baseSelect)
          ...
          // Movimientos hacia el departamento del usuario
          supabase
            .from("document_movements")
            .select(baseSelect)
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        <Card className="shadow-lg border-slate-200 dark:border-slate-700 bg-gradient-to-br from-white to-slate-50/50 dark:from-slate-800 dark:to-slate-900/50 hover:shadow-xl transition-all duration-300 hover:scale-105">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600 dark:text-slate-300">Total Documentos</CardTitle>
