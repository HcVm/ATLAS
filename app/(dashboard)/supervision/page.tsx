    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400">Cargando supervisión...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="w-full max-w-full p-4 sm:p-6 space-y-8 min-h-screen">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">
              Monitorea y gestiona las tareas de todos los empleados
            </p>
          </div>
          <div className="flex items-center space-x-2 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200/60 dark:border-slate-800/60 shadow-sm">
            <Users className="h-5 w-5 text-blue-600 dark:text-blue-400" />
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{employees.length} Empleados</span>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="boards" className="space-y-6">
          <div className="flex flex-col sm:flex-row gap-4 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm p-4 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-slate-400" />
                <Input
                  placeholder="Buscar por nombre o pizarrón..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-10 bg-white/50 dark:bg-slate-950/50 border-slate-200/60 dark:border-slate-800/60 focus:bg-white dark:focus:bg-slate-950 transition-colors"
                />
              </div>
            </div>
            <Select value={selectedEmployee} onValueChange={setSelectedEmployee}>
              <SelectTrigger className="w-full sm:w-48 bg-white/50 dark:bg-slate-950/50 border-slate-200/60 dark:border-slate-800/60">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredBoards.map((board) => {
              ...
              return (
                <Card key={board.id} className="bg-white/60 dark:bg-slate-950/60 backdrop-blur-xl border-slate-200/60 dark:border-slate-800/60 hover:shadow-xl transition-all duration-300 group">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <CardTitle className="text-lg text-slate-800 dark:text-slate-200">{board.title}</CardTitle>
                          <CardDescription className="text-sm">
                            {board.user_profile.full_name} • {board.user_profile.role}
                          </CardDescription>
                        </div>
                      </div>
                      <Badge variant="outline" className="text-xs bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                        {formatDateShort(new Date(board.created_at))}
                      </Badge>
                    </div>
                    {board.description && (
                      <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 line-clamp-2">{board.description}</p>
                    )}
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-green-50/50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/20">
                        <CheckCircle className="h-4 w-4 text-green-600 dark:text-green-400" />
                        <span className="text-slate-700 dark:text-slate-300 text-xs">
                          {stats.completed}/{stats.total} Completadas
                        </span>
                      </div>
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20">
                        <Clock className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        <span className="text-slate-700 dark:text-slate-300 text-xs">{stats.inProgress} En progreso</span>
                      </div>
                      <div className="flex items-center space-x-2 p-2 rounded-lg bg-yellow-50/50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/20">
                        <CalendarDays className="h-4 w-4 text-yellow-600 dark:text-yellow-400" />
                        <span className="text-slate-700 dark:text-slate-300 text-xs">{stats.pending} Pendientes</span>
                      </div>
                      {stats.overdue > 0 && (
                        <div className="flex items-center space-x-2 p-2 rounded-lg bg-red-50/50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20">
                          <AlertCircle className="h-4 w-4 text-red-600 dark:text-red-400" />
                          <span className="text-slate-700 dark:text-slate-300 text-xs">{stats.overdue} Vencidas</span>
                        </div>
                      )}
                    </div>

                    <div className="space-y-2">
                      <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Tareas recientes</h4>
                      {board.tasks.slice(0, 3).map((task) => (
                        <div
                          key={task.id}
                          className="flex items-center justify-between p-2.5 bg-white/50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800"
                        >
                      ...
                      {board.tasks.length > 3 && (
                        <p className="text-xs text-center text-slate-500 dark:text-slate-400 py-1">+{board.tasks.length - 3} tareas más</p>
                      )}
                    </div>
                    ...
                    <div className="flex space-x-2 pt-2">
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700"
          ...
          {filteredBoards.length === 0 && (
            <div className="text-center py-16 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
