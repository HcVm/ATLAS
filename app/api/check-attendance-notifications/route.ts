...
async function checkLateArrivals(today: string) {
  try {
    ...
    if (insertError) {
      console.error("Error creating notifications for late arrivals:", insertError)
      return
    }
    ...
    if (updateError) {
      ...
      console.log(`Late arrival notifications sent to ${lateArrivals.length} users`)
    }
  } catch (error) {
...
async function checkMissingAttendance(today: string) {
  try {
    ...
    if (absentEmployees.length === 0) {
      console.log("No absent employees to notify")
      return
    }
    ...
    if (insertError) {
      console.error("Error creating notifications for missing attendance:", insertError)
    } else {
      console.log(`Missing attendance notifications sent to ${absentEmployees.length} users`)
    }
  } catch (error) {
...
async function checkIncompleteAttendance(today: string) {
  try {
    ...
    if (insertError) {
      console.error("Error creating notifications for incomplete attendance:", insertError)
      return
    }
    ...
    if (updateError) {
      ...
      console.log(`Incomplete attendance notifications sent to ${incompleteAttendance.length} users`)
    }
  } catch (error) {
