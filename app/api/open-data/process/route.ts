  ...
  if (brandAlerts.length > 0) {
    ...
    for (let i = 0; i < brandAlerts.length; i += ALERT_BATCH_SIZE) {
      ...
      try {
        ...
        if (error) {
          console.error("Error procesando batch de alertas:", error.message)
          errors.push(`Error en batch de alertas: ${error.message}`)
        } else if (data) {
          brandAlertsCount += data.length
        }
      } catch (err) {
        console.error("Error inesperado en batch de alertas:", err)
        errors.push(`Error inesperado en batch de alertas: ${err}`)
      }
    }
  }
