-- Limpiar y reparar completamente el sistema de soporte (versión sin secuencias)
...
-- Crear algunos tickets de prueba si no existen (sin usar secuencias)
DO $$
DECLARE
    admin_user UUID;
    company_uuid UUID;
...
BEGIN
    ...
    IF admin_user IS NOT NULL THEN
        ...
        IF ticket_count = 0 THEN
            -- Crear ticket de prueba (el trigger generará el ticket_number automáticamente)
            INSERT INTO support_tickets (
                ...
                'Este es un ticket de prueba para verificar que el sistema de soporte funciona correctamente. Puedes cambiar su estado, asignarlo a personal de Tecnología y agregar comentarios.',
                'open',
                'medium',
                'system',
                admin_user,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'No se pudo crear ticket de prueba: %', SQLERRM;
END $$;

-- Verificar estructura de datos y mostrar información útil
DO $$
DECLARE
    ticket_count INTEGER;
...
BEGIN
    ...
    RAISE NOTICE 'Funciones: Recreadas correctamente';
    
    -- Mostrar un ticket de ejemplo si existe
    SELECT ticket_number, title, status INTO sample_ticket
    FROM support_tickets
    LIMIT 1;
    
    IF sample_ticket.ticket_number IS NOT NULL THEN
        RAISE NOTICE 'Ticket de ejemplo: % - % (%)', sample_ticket.ticket_number, sample_ticket.title, sample_ticket.status;
    END IF;
