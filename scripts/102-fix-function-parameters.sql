-- Corregir funciones con parámetros conflictivos

-- Eliminar funciones existentes que pueden tener conflictos de parámetros
DROP FUNCTION IF EXISTS can_manage_support_tickets(UUID, UUID);
DROP FUNCTION IF EXISTS can_manage_support_tickets(UUID);
DROP FUNCTION IF EXISTS get_tech_users(UUID);
DROP FUNCTION IF EXISTS update_ticket_status(UUID, TEXT, UUID, UUID);
...
DROP VIEW IF EXISTS tech_users_view;
CREATE VIEW tech_users_view AS
SELECT 
    p.id,
    p.full_name,
    p.email,
...
-- Función para verificar si un usuario puede gestionar tickets
CREATE FUNCTION can_manage_support_tickets(user_id UUID, ticket_company_id UUID DEFAULT NULL)
RETURNS BOOLEAN AS $$
DECLARE
    user_profile RECORD;
...
BEGIN
    ...
    -- Si se especifica ticket_company_id, verificar que coincida
...
CREATE FUNCTION get_tech_users(target_company_id UUID)
RETURNS TABLE (
    id UUID,
    full_name TEXT,
    email TEXT,
...
-- Función para actualizar estado de ticket con validaciones
...
BEGIN
    ...
    RAISE NOTICE 'Funciones eliminadas y recreadas correctamente';
